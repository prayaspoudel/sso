...
const Companies: React.FC = () => {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<Company>>({
    ...
    queryFn: async () => {
      const response = await apiClient.get(API_ENDPOINTS.COMPANIES.LIST, {
                  ...
                  <tr key={company.id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        <button className="text-blue-600 hover:text-blue-900" title="Edit company" aria-label="Edit company">
                          <Edit className="h-4 w-4" />
                        </button>
                        <button className="text-red-600 hover:text-red-900" title="Delete company" aria-label="Delete company">
